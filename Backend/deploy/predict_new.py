...
from mrcnn.model_new import MaskRCNN

from mrcnn import model_new as modellib, utils
from PIL import Image, ImageDraw
from mrcnn.model_new import load_image_gt
from mrcnn.model_new import mold_image
...
def load_model():
    ...
    model.load_weights(r'C:\Users\kmlkr\OneDrive\Pulpit\praca\mask_rcnn_marble_cfg_coco_0006.h5', by_name=True)
    return model

def process_image(img_path, model, class_names, output_dir):
    ...
    mask_coordinates = {class_id: get_mask_coordinates(filtered_results[class_id]['mask']) for class_id in filtered_class_ids}
    mask_info = {class_id: get_mask_info(coords) for class_id, coords in mask_coordinates.items()}

    display_masks(img, filtered_masks, filtered_class_ids, class_names, filtered_scores, 
                  title="Detected Bounding Boxes", figsize=(12,12), show_mask=False)
...
def main(img_dir, unq_id):
    model = load_model()
    class_names = ['BG', 'imie', 'nazwisko']
    output_dir = f"./extracted_data_{unq_id}"
    os.makedirs(output_dir, exist_ok=True)

    image_files = [os.path.join(img_dir, f) for f in os.listdir(img_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
    
    process_func = partial(process_image, model=model, class_names=class_names, output_dir=output_dir)
...
def main(img_dir):
    ...
    for i in directory:
        img = skimage.io.imread(f"{img_dir}/{i}")
