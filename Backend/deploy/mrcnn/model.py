...
def mold_image(images, config):
    ...
    colors in RGB order.
    """
    if images.shape[-1] == 3:  # RGB image
        images = np.concatenate([images, np.ones_like(images[..., :1])], axis=-1)  # Add alpha channel

    # Check if the image channels match the expected number
    if images.shape[-1] != len(config.MEAN_PIXEL):
        raise ValueError("The number of channels in the image does not match the length of MEAN_PIXEL")
